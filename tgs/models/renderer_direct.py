    ...
    def forward(self, pts, GS):
        ...
        for k, layer in zip(self.cfg.feature_channels.keys(), self.out_layers):
            if k == "rotation":
                ...
                v = torch.nn.functional.normalize(r)
            elif k == "scaling":
                ...
                if self.cfg.use_rgb:
                    v = torch.sigmoid(sh)
                v = torch.reshape(sh, (v.shape[0], -1, 3))
                ...
                v = pts if self.cfg.xyz_offset else pts
            ret[k] = v
...
class GS3DRenderer(BaseModule):
    ...
    def forward_gs(self, p, G):

        return self.gs_net(p, G)
    ...
    def forward_single_batch(
        ...
        gs: GaussianModel = self.forward_gs(query_points, GS_feats)
        out_list = []
