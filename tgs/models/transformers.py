...
from tgs.models.MLA_Attention import MLAAttention
...
class GatedSelfAttentionDense(nn.Module):
    ...
    def __init__(self, query_dim: int, latent_dim:int, context_dim: int, n_heads: int, d_head: int):
        super().__init__()

        # we need a linear projection since we need cat visual feature and obj feature
        self.linear = nn.Linear(context_dim, query_dim)

        self.attn = MLAAttention(query_dim=query_dim, latent_dim = latent_dim, heads=n_heads, dim_head=d_head)
    ...
    def __init__(
        ...
        if attention_type == "gated" or attention_type == "gated-text-image":
            self.fuser = GatedSelfAttentionDense(
                dim, latent_dim, cross_attention_dim, num_attention_heads, attention_head_dim
            )
        ...
        self.register_buffer("past_key_value_0", torch.empty(0), persistent=False)
        self.register_buffer("past_key_value_1", torch.empty(1), persistent=False)
