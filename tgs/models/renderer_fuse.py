...
def points_to_occupied_grid_centers(points: torch.Tensor, grid_size: int) -> list[torch.Tensor]:
    """
    将归一化到 [-1, 1] 范围内的点云映射到指定尺寸的占用网格，
    并返回每个被点云占用的网格的中心坐标。

    Args:
        points: 形状为 (B, N, 3) 的张量，点云坐标，已归一化到 [-1, 1]。
                其中 B 是批次大小，N 是每个点云的点数，3 是坐标维度 (x, y, z)。
        grid_size: 整数，网格在每个维度上的划分数量。例如，grid_size=64
                   意味着创建一个 64x64x64 的网格。

    Returns:
        一个列表，列表的长度等于批次大小 B。列表中的每个元素是一个张量，
        包含了该批次项中所有被占用的网格的中心坐标，形状为 (N_occupied, 3)，
        其中 N_occupied 是该批次项中被占用的网格数量。注意 N_occupied
        对于不同的批次项可能是不同的。
    """
    B, N, C = points.shape
    if C != 3:
        raise ValueError("输入点云张量的最后一个维度必须是 3 (x, y, z 坐标)。")
    if not isinstance(grid_size, int) or grid_size <= 0:
        raise ValueError("grid_size 必须是一个正整数。")

    # 将点云坐标从 [-1, 1] 映射到 [0, grid_size - 1] 的网格索引
    # 步骤：
    # 1. 将 [-1, 1] 映射到 [0, 2]： coord + 1
    # 2. 将 [0, 2] 映射到 [0, 1]： (coord + 1) / 2
    # 3. 将 [0, 1] 映射到 [0, grid_size]： ((coord + 1) / 2) * grid_size
    # 4. 取整得到网格索引： floor(...)
    # 5. 确保索引在 [0, grid_size - 1] 范围内： clamp(...)

    # 为了方便处理，我们将所有批次和点的维度展平
    points_flat = points.view(B * N, C) # 形状 (B*N, 3)

    # 计算浮点网格坐标
    grid_coords_float = (points_flat + 1.0) / 2.0 * grid_size

    # 计算整数网格索引
    grid_indices = torch.floor(grid_coords_float).long()

    # 将索引限制在 [0, grid_size - 1] 范围内
    grid_indices = torch.clamp(grid_indices, 0, grid_size - 1)

    # 为展平后的点创建对应的批次索引
    batch_indices = torch.arange(B, device=points.device).repeat_interleave(N) # 形状 (B*N,)

    # 将批次索引和网格索引结合起来，形成 (batch_idx, i, j, k) 元组
    # 元组形状为 (B*N, 4)
    full_indices = torch.cat([batch_indices.unsqueeze(1), grid_indices], dim=1)

    # 找到唯一的 (batch_idx, i, j, k) 元组，这些代表被占用的网格
    # unique 会自动去除重复项，并返回唯一的行
    unique_indices, _ = torch.unique(full_indices, dim=0, return_inverse=True) # 形状 (N_unique, 4)

    # 分离唯一的批次索引和唯一的网格索引
    unique_batch_indices = unique_indices[:, 0] # 形状 (N_unique,)
    unique_grid_indices = unique_indices[:, 1:] # 形状 (N_unique, 3)

    # 计算这些被占用网格的中心坐标
    # 网格单元的大小是 2.0 / grid_size
    # 网格 (i, j, k) 的中心在 [-1, 1] 范围内的坐标是 -1 + (index + 0.5) * cell_size
    cell_size = 2.0 / grid_size
    # 形状 (N_unique, 3)
    occupied_cell_centers_flat = -1.0 + (unique_grid_indices.float() + 0.5) * cell_size

    # 根据批次索引分割结果
    output_list = []
    for b in range(B):
        # 选择属于当前批次项的中心点
        mask = (unique_batch_indices == b)
        centers_for_batch = occupied_cell_centers_flat[mask]
        output_list.append(centers_for_batch)
    ...
    def configure(self, *args, **kwargs) -> None:
        ...
        for key, out_ch in self.cfg.feature_channels.items():
            if key == "shs" and self.cfg.use_rgb:
                out_ch = 3
            layer = nn.Linear(self.cfg.in_channels, out_ch * self.cfg.generate_points)
            # initialize
        ...
        self.weight = torch.tensor([
            0.23873241, 0.23873241, 0.23873241,          # l=1
            0.39788736, 0.39788736, 0.39788736, 0.39788736, 0.39788736,  # l=2
            0.55604231, 0.55604231, 0.55604231, 0.55604231, 0.55604231, 0.55604231, 0.55604231  # l=3
        ], device='cuda')

    def forward(self, x, pts, GS):
        ret = {}

        for k, layer in zip(self.cfg.feature_channels.keys(), self.out_layers):
            v = layer(x)
            v = v.reshape(v.size(0) * self.cfg.generate_points, v.size(1) // self.cfg.generate_points)
            if k == "rotation":
                r = torch.repeat_interleave(GS[:, 1:5], repeats=self.cfg.generate_points, dim=0)
                v = torch.nn.functional.normalize(v + r)
            elif k == "scaling":
                s = torch.repeat_interleave(GS[:, 5:8], repeats=self.cfg.generate_points, dim=0)
                v = trunc_exp(v + s)
                if self.cfg.clip_scaling is not None:
                    v = torch.clamp(v, min=0, max=self.cfg.clip_scaling)
            elif k == "opacity":
                o = torch.repeat_interleave(GS[:, 0:1], repeats=self.cfg.generate_points, dim=0)
                v = torch.sigmoid(v + o)
            elif k == "shs":
                sh = torch.repeat_interleave(GS[:, 8:], repeats=self.cfg.generate_points, dim=0)
                if self.cfg.use_rgb:
                    v = torch.sigmoid(v + sh)
                v = torch.reshape(v + sh, (v.shape[0], -1, 3))

                # v_clone = v.clone() 

                # v[:, 0, :] = torch.sigmoid(v[:, 0, :])
                # v[:, 0, :]  = v[:, 0, :] - 0.5

                # delta = 0.5 - torch.abs(v[:, 0, :]) 
                # max_high_norm = delta**2 
                # v[:, 0, :]  = v[:, 0, :]  / 0.282

                # weighted_sq_high = v[:, 1:, :]**2 * self.weight.unsqueeze(0).unsqueeze(-1)
                # sum_high = torch.sum(weighted_sq_high, dim=1, keepdim=True) + 1e-6
                # scale_high = max_high_norm / sum_high.squeeze(dim=1)
                # scale_high = scale_high.sqrt()
                # scale_high = torch.clamp(scale_high, max=1.0)
                # #pdb.set_trace()

                # v_clone[:, 1:, :] = v[:, 1:, :] * scale_high.unsqueeze(1)
                ...
                pts = torch.repeat_interleave(pts, repeats=self.cfg.generate_points, dim=0)
...
class GS3DRenderer(BaseModule):
    ...
    def configure(self, *args, **kwargs) -> None:
        ...
        # mlp_in = self.cfg.projection_feature_dim
    ...
    def forward_gs(self, x, p, G):
        if self.cfg.mlp_network_config is not None:
            
            x = self.mlp_net(x)

        return self.gs_net(x, p, G)
    ...
    def forward_single_view(self,
        ...
        bg_color = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32, device=self.device)
        ...
        if self.gs_net.cfg.use_rgb:
            ...
            shs = gs.shs
        #print(f"shs = {(shs_mask + 1)**2}" )
        if shs is not None:
            shs[:, (shs_mask + 1)**2:] = 0
        #print(shs[:, (shs_mask + 1)**2:])
        # pdb.set_trace()
        ...
        ret = {
            "comp_rgb": rendered_image.half().permute(1, 2, 0),
        ...
        if ret_mask:
            ...
            with torch.autocast(device_type=self.device.type, dtype=torch.float32):
                rendered_mask, radii, depth = rasterizer(
                    means3D = means3D,
                    means2D = means2D,
                    # shs = ,
    ...
    def query_triplane(
        ...
        if scene_bounds is None:
            ...
            positions_x = scale_tensor(positions[:, :, 0], (x_min, x_max), (-1, 1))
            positions_y = scale_tensor(positions[:, :, 1], (y_min, y_max), (-1, 1))
            positions_z = scale_tensor(positions[:, :, 2], (z_min, z_max), (-1, 1))
    ...
    def forward_single_batch(
        ...
        GS_feats: Float[Tensor, "Np 56"],
        c2ws: Float[Tensor, "Nv 4 4"],
        intrinsics: Float[Tensor, "Nv 4 4"],
        height: int,
        width: int,
        background_color: Optional[Float[Tensor, "3"]],
        shs_mask: Optional[int] = 0
    ):

        gs: GaussianModel = self.forward_gs(gs_hidden_features, query_points, GS_feats)
        out_list = []
    ...
    def forward(self, 
        ...
        GS_feats: Optional[Float[Tensor, "B C H W"]] = None,
        ...
        **kwargs):
        ...
        gs_hidden_features = self.query_triplane(query_points, gs_hidden_features, scene_bounds)

        
        if additional_features is not None:
            gs_hidden_features = torch.cat([gs_hidden_features, additional_features], dim=-1)
            # gs_hidden_features = additional_features
