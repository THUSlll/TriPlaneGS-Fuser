...
class ImageTriplaneGenerator(nn.Module):
    def __init__(self, grid_size=128, n_heads=4, feature_channels=196, num_samples=8, num_layer=2):
        super().__init__()
            ...
            for b in range(B):
                for v in range(Nv):
                    world_points, mask = project_depth_to_3d(
                        depth_cond[b, v], c2w_cond[b, v], intrinsic_cond[b, v]
                    )
            ...
            for b in range(B):
                ...
                for i, plane in enumerate(['xy', 'xz', 'yz']):
                    ...
                    for v in range(Nv):
                        ...
                        ind = torch.full(
                            (feature.size(1),),
                            v * self.grid_size * self.grid_size, # 唯一的 (批次, 视图) 组合索引
                            dtype=torch.long,
                    ...
                    feature_collector = torch.zeros(
                        (Nv * self.grid_size * self.grid_size, self.feature_channels),
                        device=features.device,
                        dtype=features.dtype
                    )
                    count_collector = torch.zeros(
                        (Nv * self.grid_size * self.grid_size),
                        device=features.device,
                        dtype=features.dtype
                    )
                    ...
                    features_flat = feature_collector.view(Nv, -1 , self.feature_channels)  # [H*W, C]
                    features_flat = features_flat.permute(1, 0, 2)
                    
                    # 对投影特征也进行归一化
                    features_flat = self.norm1_cross_view[n](features_flat)

                    # 计算注意力
                    attn_output, prob = self.attentions_corss_view[n](
                        queries.unsqueeze(0),  # [1, H*W, C]
                        features_flat.unsqueeze(0) # [1, H*W, Nv, C]
                    )  # [1, H*W, C]
                ...
                for i, plane in enumerate(['xy', 'xz', 'yz']):
                    ...
                    other_plane_features = self.norm1_cross_plane[n](other_plane_features).view(-1, 16, self.feature_channels)
