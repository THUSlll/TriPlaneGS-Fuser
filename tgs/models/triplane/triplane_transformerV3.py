...
from diff_gaussian_rasterization_ch228 import GaussianRasterizer, GaussianRasterizationSettings
...
class ImageTriplaneGenerator(nn.Module):
    def __init__(self, grid_size=128, n_heads=4, feature_channels=196, num_samples=4, num_layer=2):
        ...
        self.raster_settings_XY = GaussianRasterizationSettings(
        ...
        self.raster_settings_YZ = GaussianRasterizationSettings(
        ...
        self.raster_settings_XZ = GaussianRasterizationSettings(
    ...
    def Splatting_Points_to_Plane(self, feature, pts):
        ...
        colors_precomp_count = torch.ones_like(colors_precomp, dtype=torch.float32, device="cuda")
        ...
        for plane in ['xy', 'xz', 'yz']:
            # 选择对应的坐标轴
            if plane == 'xy':
                rasterizer = GaussianRasterizer(self.raster_settings_XY)
            if plane == 'xz':
                rasterizer = GaussianRasterizer(self.raster_settings_XZ)
            if plane == 'yz':
                rasterizer = GaussianRasterizer(self.raster_settings_YZ)           
            ...
            count_image, radii, depth = rasterizer(
                means3D = coords,
                means2D = means2D,
                shs = shs,
            ...
            tri_planes[plane] = (rendered_image / count_image).unsqueeze(0)
    ...
    def forward(self, image_features, depth_cond, c2w_cond, intrinsic_cond, depth, c2w, intrinsic, sample_grid, scene_bounds=None):
        ...
        for b in range(B):
            ...
            print(bounded_points[mask].size(0))
            start_timer(f"splat")
            for v in range(Nv):
                ...
                tri_plane = self.Splatting_Points_to_Plane(feature, pts)

                splat_feat = torch.cat([tri_plane['xy'].unsqueeze(1), tri_plane['xz'].unsqueeze(1), tri_plane['yz'].unsqueeze(1)], dim=1)

                splat_feats.append(splat_feat.unsqueeze(2))
            end_timer(f"splat")
            splat_feats = torch.cat(splat_feats, dim=2)
