...
class Encoder(nn.Module):  ## Embedding module
    ...
    def forward(self, point_groups):
        '''
            point_groups : B G N in_dim
            -----------------
            feature_global : B G C
        '''
        bs, g, n, d = point_groups.shape
        point_groups = point_groups.reshape(bs * g, n, d)
        # encoder
...
class Group(nn.Module):  # FPS + KNN
    def __init__(self, num_group, group_size, in_dim=3):
        super().__init__()
    ...
    def forward(self, xyz):
        '''
            ...
            center : B G in_dim
        '''
        batch_size, num_points, d = xyz.shape
        
        # 如果点云数量过大，先进行下采样
        max_points = 500000  # 设置最大点云数量
        if num_points > max_points:
            tqdm.write(f"点云数量({num_points})过大，下采样到{max_points}")
            # 随机下采样
            indices = torch.randperm(num_points, device=xyz.device)[:max_points]
            xyz = xyz[:, indices, :]
            num_points = max_points
        
        center = misc.fps(xyz, self.num_group)  # B G in_dim
        try:
            _, idx = self.knn(xyz, center)  # B G M
        except RuntimeError as e:
            tqdm.write("KNN CUDA RuntimeError:", e)
            # 安全地获取 shape 信息，避免访问损坏的内存
            try:
                tqdm.write("xyz.shape:", xyz.shape)
                tqdm.write("center.shape:", center.shape)
                tqdm.write("xyz.device:", xyz.device, "center.device:", center.device)
            except:
                tqdm.write("无法获取 shape 信息, CUDA 内存可能已损坏")
            
            # 检查 k 值
            tqdm.write("self.k (group_size):", self.group_size)
            tqdm.write("self.num_group:", self.num_group)
            
            # 检查输入数据是否合理
            try:
                if xyz.shape[1] < self.group_size:
                    tqdm.write(f"错误：点云数量({xyz.shape[1]}) < k值({self.group_size})")
                if center.shape[1] < self.num_group:
                    tqdm.write(f"错误：中心点数量({center.shape[1]}) < 组数({self.num_group})")
            except:
                tqdm.write("无法检查数据合理性")
            
            # 不重新抛出异常，而是返回默认值或跳过
            tqdm.write("跳过 KNN 操作，使用默认值")
            
            # 提供一个合理的回退方案：随机选择邻居
            batch_size = xyz.shape[0] if hasattr(xyz, 'shape') else 1
            num_points = xyz.shape[1] if hasattr(xyz, 'shape') else 0
            
            # 为每个中心点随机选择 group_size 个邻居
            idx = torch.randint(0, num_points, (batch_size, self.num_group, self.group_size), device=xyz.device)
        ...
        neighborhood = neighborhood.view(batch_size, self.num_group, self.group_size, d).contiguous()
...
class PointMambaScan(nn.Module):
    def __init__(self, config, in_dim=59, **kwargs):
        ...
        self.in_dim = in_dim
        self.group_divider = Group(num_group=self.num_group, group_size=self.group_size, in_dim=in_dim)
        self.encoder = Encoder(encoder_channel=self.encoder_dims, in_dim=in_dim)
    ...
    def forward(self, pts):
        ...
        G = x.shape[1] // 2
        x_forward = x[:, :G, :]      # [B, G, C]
        x_backward = x[:, G:, :]     # [B, G, C]
        x_avg = 0.5 * (x_forward + x_backward)  # [B, G, C]

        # neighborhood: [B, G, M, 3]
        # center: [B, G, 3]

        dist = torch.norm(neighborhood[..., :3], dim=-1)  # [B, G, M]
        var = dist.var(dim=2)          # [B, G]
        std = torch.sqrt(var + 1e-6)   # 防止除零

        # 计算高斯权重
        # dist: [B, G, M], std: [B, G] -> 需要扩展std到 [B, G, M]
        std_expanded = std.unsqueeze(-1)  # [B, G, 1]
        gaussian_weight = torch.exp(-0.5 * (dist / (std_expanded + 1e-6)) ** 2).reshape(B, G * M)  # [B, G * M]

        neighbor_xyz = neighborhood[..., :3] + center.unsqueeze(2).repeat(1,1,self.group_size,1)  # [B, G, M, 3]

        dense_point = neighbor_xyz.reshape(B, -1, 3)  # [B, G * M, 3]

        dense_feat = x_avg.unsqueeze(2).repeat(1,1,self.group_size,1).reshape(B, -1, self.trans_dim) 

        dense_feat = torch.cat([dense_point, dense_feat], dim=-1)

        return x_avg, dense_feat, gaussian_weight
