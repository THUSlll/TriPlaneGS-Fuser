...
if __name__ == "__main__":
    ...
    model_path = "exp/test_2025_7_14/checkpoint/model_checkpoint_epoch_0_step_0.pth"
    ...
    for epoch in tqdm(range(0, num_epochs), desc="Total Epochs"):
        ...
        for batch_idx in tqdm(range(0, 190), desc=f"Training Epoch {epoch}/{num_epochs}"):
            try:
            ...
            # 查看 model.camera_embadder 的梯度信息
            # for name, param in model.camera_embedder.named_parameters():
            #     if param.grad is not None:
            #         tqdm.write(f"camera_embedder: {name}, 梯度均值: {param.grad.mean().item()}, 梯度最大值: {param.grad.max().item()}, 梯度最小值: {param.grad.min().item()}")
            #     else:
            #         tqdm.write(f"camera_embedder: {name}, 梯度为 None")
                    
            scaler.step(optimizer)
            scaler.update()

            # 使用tqdm.write替代print <button class="citation-flag" data-index="8">
            tqdm.write(f"Epoch [{epoch}/{num_epochs}], Batch [{batch_idx}/{190}], L1 Loss: {L1.item()}, SSIM Loss: {(1.0 - ssim_value).item()}, Scene: {batch['scene_path'][0].split('/')[-2]}, PSNR: {val.item()}, shs_mask:{shs_mask}")
            iteration += 1
        ...
        with torch.no_grad():
            ...
            ssims = []
            lpipss = []
            for batch_idx in tqdm(range(190, 199), desc=f"Training Epoch {epoch}/{num_epochs}"):
                torch.cuda.empty_cache()  
                try:
                    batch = next(train_iter)
                except StopIteration:
