...
from tgs.models.triplane.point_triplane_temp import PointTriplaneGenerator
...
class TGS(torch.nn.Module, SaverMixin):
    ...
    def forward(self, batch: Dict[str, Any], shs_mask:Optional[int] = 0) -> Dict[str, Any]:
        ...
        # )
        # pdb.set_trace()
        image_scene_codes, scene_bounds = self.triplaneGenerator(image_features, depth_cond, c2w_cond, intrinsic_cond, pointclouds)

        point_scene_codes = self.pointTriplaneGenerator(torch.cat([pointclouds, GS_feats], dim=-1), scene_bounds)
        ...
        rend_out = self.renderer(scene_codes,
                                query_points=opt_pointclouds[0],
...
if __name__ == "__main__":
    ...
    # model_path = "exp/test_2025_3_21_2/checkpoint/model_checkpoint_epoch_50_step_0.pth"
