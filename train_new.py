...
import numpy as np
import pdb
from tgs.models.triplane.triplane import TriplaneGenerator
...
class TGS(torch.nn.Module, SaverMixin):
    ...
    def __init__(self, cfg):
        ...
        self.backbone_xy = tgs.find(self.cfg.backbone_cls)(self.cfg.backbone)

        self.backbone_yz = tgs.find(self.cfg.backbone_cls)(self.cfg.backbone)

        self.backbone_xz = tgs.find(self.cfg.backbone_cls)(self.cfg.backbone)
        ...
        self.triplaneGenerator = TriplaneGenerator(128)
    ...
    def forward(self, batch: Dict[str, Any], shs_mask:Optional[int] = 0) -> Dict[str, Any]:
        ...
        # proj_feats = points_projection(pointclouds, c2w_cond, intrinsic_cond, image_feature)
        ...
        # tokens_xy = self.backbone_xy(
        #     self.triPlaneEbedding_xy,
        #     input_image_tokens,
        #     modulation_cond=None,
        # )

        # tokens_yz = self.backbone_yz(
        #     self.triPlaneEbedding_yz,
        #     input_image_tokens,
        #     modulation_cond=None,
        # )

        # tokens_xz = self.backbone_xz(
        #     self.triPlaneEbedding_xz,
        #     input_image_tokens,
        #     modulation_cond=None,
        # )

        # tokens = torch.concat([tokens_xy, tokens_yz, tokens_xz], dim = 1)
        ...
        scene_codes = self.triplaneGenerator(image_feature, depth_cond, c2w_cond, intrinsic_cond)
        rend_out = self.renderer(scene_codes,
                                query_points=pointclouds,
