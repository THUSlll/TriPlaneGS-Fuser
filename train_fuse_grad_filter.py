...
import numpy as np
import pdb
from tgs.models.triplane.triplane_transformerV2 import ImageTriplaneGenerator
...
class TGS(torch.nn.Module, SaverMixin):
    ...
    def __init__(self, cfg):
        ...
        # self.triplaneGenerator = ImageTriplaneGenerator(grid_size=196, feature_channels=228)
        # self.pointTriplaneGenerator = PointTriplaneGenerator(196, in_channels=384, out_channels=228)
    ...
    def forward(self, batch: Dict[str, Any], shs_mask:Optional[int] = 0) -> Dict[str, Any]:
        ...
        with torch.no_grad():
            for id in range(batch['part_num'][0]):
                ...
                GS_feat = torch.cat([batch["opacities"][id], batch["rots"][id], batch["scales"][id], rgb_feats], dim=-1)
                GS_feats.append(GS_feat[batch["opt_mask"][id]])

        # omega, fused_scene_code = self.FuseNet(scene_codes)
        ...
        # recent_ply_feat = torch.cat([recent_ply_feat, GS_embedding], dim=-1)
        ...
        # fused_scene_code = torch.zeros([1, 3, 228, 128, 128], dtype=torch.float32, device=recent_ply.device)
    ...
    def forward_single_part(self, id, batch: Dict[str, Any], scene_bounds, shs_mask:Optional[int] = 0) -> Tensor:
        ...
        point_feats = torch.cat([batch['opt_point_cloud'][id], opt_GS_feats], dim=-1)
        ...
        gate_weight = rearrange(gate_weight, 'B (P H W) C -> B P C H W', H = 196, W=196, P=3)
        ...
        return scene_codes, central_point_feature, pointclouds, proj_feats, GS_feats
...
if __name__ == "__main__":
    ...
    model_path = "exp/test_2025_7_12/checkpoint/model_checkpoint_epoch_0_step_0.pth"
    ...
    for epoch in tqdm(range(0, num_epochs), desc="Total Epochs"):
        ...
        # for batch_idx in tqdm(range(0, 5000), desc=f"Training Epoch {epoch}/{num_epochs}"):
        #     try:
        ...
        #         rendered_image = outputs['comp_rgb'][0] 

        #         gt_image = batch['rgb'].permute(0, 1, 3, 4, 2).squeeze(1) 
        ...
        #     tqdm.write(f"Epoch [{epoch}/{num_epochs}], Batch [{batch_idx}/{5000}], L1 Loss: {L1.item()}, SSIM Loss: {(1.0 - ssim_value).item()}, Scene: {batch['scene_path'][0].split('/')[-2]}, PSNR: {val.item()}, shs_mask:{shs_mask}")
        #     writer.add_scalar('train/loss', loss.item(), iteration)
        #     writer.add_scalar('train/L1', L1.item(), iteration)
        #     writer.add_scalar('train/SSIM', ssim_value.item(), iteration)
        #     writer.add_scalar('train/PSNR', val.item(), iteration)
        #     iteration += 1
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm(test_dataloader, desc=f"Validation Epoch {epoch}")):  # [[新增]]
                ...
                psnrs.append(psnr(gt, render).cpu())
                # psnr_scene.clear()
                scene.append(batch["scene_path"][0])
...
#                             sigma = 1.0
#                             # 创建高斯核
#                             x = torch.arange(-(kernel_size // 2), kernel_size // 2 + 1, dtype=torch.float32, device=alpha_mask.device)
#                             y = torch.arange(-(kernel_size // 2), kernel_size // 2 + 1, dtype=torch.float32, device=alpha_mask.device)
...
#                             # 对每个通道分别进行卷积
#                             opacity_masks = alpha_mask.unsqueeze(1)  # 添加通道维度 [B, 1, H, W]
#                             opacity_masks = torch.nn.functional.conv2d(opacity_masks, gaussian_kernel, padding=kernel_size//2)
#                             opacity_masks = opacity_masks.squeeze(1)  # 移除通道维度 [B, H, W]
# ##############################################

#                             mask = (opacity_masks != 0)
#                             mask = mask.repeat(3, 1, 1)  # 重复3次以匹配RGB通道
#                             render[mask] = 0
                ...
                # alpha_mask = rearrange(alpha_mask, 'C H W -> H W C') 

                img_np = (img.detach().cpu().numpy() * 255).astype('uint8')
                # alpha_mask_np = (alpha_mask.detach().cpu().numpy() * 255).astype('uint8')
                # # 将单通道mask转换为3通道RGB图像
                # alpha_mask_np = np.repeat(alpha_mask_np, 3, axis=-1)
                ...
                img_save.save(img_path)
                    # alpha_mask_save.save(alpha_mask_path)
            ...
            pdb.set_trace()
            file_path = "/hdd/u202320081001061/ScanNet-GSReg/test/drop_scene.txt"
            mask = np.array(psnrs)<19.0
            scene_np = np.array(scene)
            del_scene = scene_np[mask] # 现在这行可以正常工作了，因为 scene_np 是 NumPy 数组

            with open(file_path, "w") as file:
                for item in del_scene:
                    file.write(item + "\n") # 在每个字符串后面添加换行符
