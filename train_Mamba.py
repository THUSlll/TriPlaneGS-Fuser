...
class TGS(torch.nn.Module, SaverMixin):
    ...
    def forward(self, batch: Dict[str, Any], shs_mask:Optional[int] = 0) -> Dict[str, Any]:
        ...
        rgb_feats = torch.cat([batch["features_dc"], batch["features_extra"]], dim=-1).transpose(2, 3).flatten(2)
        GS_feats = torch.cat([batch["opacities"], batch["rots"], batch["scales"], rgb_feats], dim=-1)
        ...
        #pdb.set_trace()
        rgb_cond_flat = rearrange(batch["rgb_cond"], 'b nv c h w -> (b nv) c h w')
        ...
        depth_cond = batch['depth_cond']
        # # print(pointclouds.size())
        # print(c2w_cond.size())
        # print(intrinsic_cond.size())
        # # print(image_features.size())

        # proj_feats = points_projection(batch["opt_point_cloud"], c2w_cond, intrinsic_cond, image_features)
        ...
        point_feats = torch.cat([pointclouds, GS_feats], dim=-1)
        point_feats = rearrange(point_feats, 'B N C -> B C N')
        local_point_feature, global_point_feature = self.pointNet(point_feats)

        render_point, image_scene_codes, scene_bounds = self.triplaneGenerator(image_features, depth_cond, c2w_cond, intrinsic_cond, batch["depth"], batch["c2w"], batch["intrinsic"])
        local_point_feature = rearrange(local_point_feature, 'B C N -> B N C')
        point_scene_codes = self.pointTriplaneGenerator(torch.cat([pointclouds, local_point_feature], dim=-1), scene_bounds)
        scene_codes = torch.cat([image_scene_codes, point_scene_codes], dim=2)

        # scene_codes = rearrange(scene_codes, 'B N C H W -> B N H W C') 
        # imgxy = (scene_codes[0, 0].detach().cpu().numpy() * 255).astype('uint8')
        # imgxz = (scene_codes[0, 1].detach().cpu().numpy() * 255).astype('uint8')
        # imgyz = (scene_codes[0, 2].detach().cpu().numpy() * 255).astype('uint8')
        #                 #pdb.set_trace()
        # img_xy_save = Image.fromarray(imgxy)
        # img_xz_save = Image.fromarray(imgxz)
        # img_yz_save = Image.fromarray(imgyz)
        # img_xy_save.save('xy.png')
        # img_xz_save.save('xz.png')...................................................................................................................................................................................................................................................................................................................................................................................................................................................
        # img_yz_save.save('yz.png')
        # pdb.set_trace()
        # with torch.no_grad():
        #     opt_pointclouds = batch["opt_point_cloud"]

        #     opt_pointclouds = grid_sample_centers(opt_pointclouds, scene_bounds)
        ...
        rend_out = self.renderer(
                                image_scene_codes,
                                query_points=batch["opt_point_cloud"],
                                # additional_features = proj_feats,
                                GS_feats=GS_feats.unsqueeze(0)[batch["opt_mask"]].unsqueeze(0),
                                shs_mask = shs_mask,
            ...
            with torch.no_grad():
                ...
                for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Validation Epoch {epoch}")):  # [[新增]]
                    if vali_idx % 20 == 0:
                        ...
                        if epoch % 5 == 0 and vali_idx % 40 == 0:
